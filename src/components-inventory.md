...
### BottomTabBar.tsx
- **역할**: 하단 탭바 (홈/나다움/마음톡/프로필)
- **사용처**: HomeScreenNew, 전역 레이아웃
- **타입**: Layout Component
- **주요 기능**:
  - 4개 탭: 홈, 나다움(나다움 분석), 마음톡(AI 채팅), 프로필
  - 활성 탭 하이라이트
- **파일 경로**: `/components/BottomTabBar.tsx`
- **추가일**: 2026-03-10
...
12. **ConsultTypeSelectPage.tsx** (/consult-type-select)
    - **역할**: 상담 유형 선택 (사주/타로)
    - **파일 경로**: `/pages/ConsultTypeSelectPage.tsx`
    - **추가일**: 2026-03-11
...
> 나다움 DNA 분석 시각화 (2026-03-10 추가).

### NadaumAnalysisPage.tsx
- **역할**: 나다움 분석 메인 페이지
- **사용처**: `/nadaum` 라우트
- **타입**: Page Component
- **주요 기능**:
  - 나다움 유형 카드 (MBTI 스타일 16유형 + 그라디언트 + 명언 + 공유)
  - 오행 에너지 수평 바 차트 (만세력 API `발달오행` 기반)
  - 나다움 태그 1줄 가로 스크롤
  - 상세 분석 카드 (초대 해금 기반)
- **파일 경로**: `/components/NadaumAnalysisPage.tsx`
- **추가일**: 2026-03-10

### NadaumAnalysisDetail.tsx
- **역할**: 나다움 상세 분석 페이지
- **사용처**: `/nadaum/:category` 라우트
- **타입**: Page Component
- **주요 기능**:
  - ScoreGauge (반원형 0-100 점수)
  - SpectrumBar (2축 스펙트럼)
  - AI JSON 포맷 (text, score, spectrum)
- **파일 경로**: `/components/NadaumAnalysisDetail.tsx`
- **추가일**: 2026-03-10
...
- **주요 기능**:
  ...
  - AI 선제 인사 (심리 데이터 기반)
  - Gemini 2.5 Flash SSE 스트리밍
  - 타로 카드 뽑기 (7장 중 3장 선택 → AI 해석 오버레이)
  - 라운드 시스템 (새로고침으로 새 세션)
  - 새싹 부족 시 SproutChargingStation 오버레이
  - 4주 심리 흐름 (user_situation_summaries)
- **파일 경로**: `/pages/MindTalkPage.tsx`
- **추가일**: 2026-03-10
...
- 에러 처리: 2개
- 공유 리워드: 2개
- 홈 고도화: 17개 (+ConsultTypeSelectPage)
- 사주/타로 상담 체험: 13개
...
### 2026-03-11
- **나다움 분석 섹션 추가** (106 → 108개 활성)
  - NadaumAnalysisPage, NadaumAnalysisDetail
- **마음톡 섹션 추가** (108 → 109개 활성)
  - MindTalkPage (AI 채팅, 모드 시스템, SSE 스트리밍)
- **BottomTabBar 추가** (109 → 110개 활성)
  - 하단 탭바 (홈/나다움/마음톡/프로필)
- **ConsultTypeSelectPage 추가** (110 → 112개 활성)
  - 상담 유형 선택 (사주/타로)
