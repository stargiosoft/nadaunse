...
| `/search` | 검색 \| 나다운세 |
| `/consult-type-select` | 상담 유형 선택 \| 나다운세 |
...
| 38 | `consult_start_click` | `trackConsultStartClick` | 상담 유형 선택 페이지에서 사주/타로 선택 클릭 |
| 38-1 | `consult_entry_click` | `trackConsultEntryClick` | 홈에서 "마음 털어놓기" 버튼 클릭 (상담 진입) |
| 38-2 | `consult_type_select_view` | `trackConsultTypeSelectView` | 상담 유형 선택 페이지 조회 |
...
> **데일리 마음 상담 퍼널**: 홈 "마음 털어놓기"(`consult_entry_click`) → 유형 선택 페이지(`consult_type_select_view`) → 사주/타로 선택(`consult_start_click`) → 질문 제출(`consult_submit`) → 결과 → 추천 콘텐츠(`consult_recommendation_click`)
> - `consult_entry_click`: `source` 파라미터 (`home_new` 신규 상담 / `home_result` 결과 보기)
...
| # | 이벤트명 | 함수명 | 설명 |
|---|---------|--------|------|
| 41 | `nadaum_page_view` | `trackNadaumPageView` | 나다움 분석 메인 페이지 조회 |
| 42 | `nadaum_category_click` | `trackNadaumCategoryClick` | 카테고리 카드 클릭 (잠금/해제 구분) |
| 43 | `nadaum_analysis_view` | `trackNadaumAnalysisView` | 나다움 분석 상세 조회 (AI 분석 완료 시) |
| 44 | `nadaum_refresh` | `trackNadaumRefresh` | 상세에서 "다시 분석" 클릭 |
| 45 | `nadaum_dna_refresh` | `trackNadaumDnaRefresh` | 메인에서 DNA 리포트 새로고침 |
| 46 | `nadaum_login_click` | `trackNadaumLoginClick` | 비로그인 시 로그인 유도 클릭 |
| 47 | `nadaum_collect_tag_click` | `trackNadaumCollectTagClick` | "운세 보고 태그 모으기" CTA 클릭 |

> - `nadaum_page_view`: `is_logged_in`, `tag_count`, `is_unlocked` → 진입 시 잠금 해제 여부 분석
> - `nadaum_category_click`: `category` (love/nature/money/career/health), `is_unlocked` → 잠긴 카드 클릭도 추적
> - `nadaum_analysis_view`: `category`, `tag_count`, `is_cached` → 캐시 vs 신규 생성 비율 분석
> - `nadaum_collect_tag_click`: `source` (main/category) → 태그 모으기 전환 퍼널 분석

### 마음톡 이벤트

| # | 이벤트명 | 함수명 | 설명 |
|---|---------|--------|------|
| 48 | `mindtalk_page_view` | `trackMindTalkPageView` | 마음톡 페이지 조회 |
| 49 | `mindtalk_mode_change` | `trackMindTalkModeChange` | 모드 변경 (일반/사주/타로) |
| 50 | `mindtalk_message_send` | `trackMindTalkMessageSend` | 메시지 전송 |
| 51 | `mindtalk_tarot_draw` | `trackMindTalkTarotDraw` | 타로 카드 뽑기 완료 |
| 52 | `mindtalk_sprout_insufficient` | `trackMindTalkSproutInsufficient` | 새싹 부족 (충전소 표시) |
| 53 | `mindtalk_login_click` | `trackMindTalkLoginClick` | 비로그인 시 로그인 유도 클릭 |
| 54 | `mindtalk_new_round` | `trackMindTalkNewRound` | 새 대화 시작 (라운드 증가) |

> - `mindtalk_message_send`: `mode`, `round`, `is_suggestion` (추천 질문 여부), `is_paid` (새싹 소모 여부) → 모드별 메시지 전환율 분석
> - `mindtalk_sprout_insufficient`: `mode`, `current_balance` → 이탈 원인 분석
> - `mindtalk_mode_change`: `mode` → 모드별 선호도 분석
...
| `HomeScreenNew.tsx` | `consult_entry_click`, `consult_login_click` |
| `ConsultTypeSelectPage.tsx` | `consult_type_select_view`, `consult_start_click`, `consult_login_click` |
...
| `NadaumAnalysisPage.tsx` | `nadaum_page_view`, `nadaum_category_click`, `nadaum_dna_refresh`, `nadaum_login_click`, `nadaum_collect_tag_click` |
| `NadaumAnalysisDetail.tsx` | `nadaum_analysis_view`, `nadaum_refresh` |
| `MindTalkPage.tsx` | `mindtalk_page_view`, `mindtalk_mode_change`, `mindtalk_message_send`, `mindtalk_tarot_draw`, `mindtalk_sprout_insufficient`, `mindtalk_login_click` |
...
| 2026-03-11 | 나다움 분석 이벤트 추가 (41-47번: `nadaum_page_view`, `nadaum_category_click`, `nadaum_analysis_view`, `nadaum_refresh`, `nadaum_dna_refresh`, `nadaum_login_click`, `nadaum_collect_tag_click`) |
| 2026-03-11 | 마음톡 이벤트 추가 (48-54번: `mindtalk_page_view`, `mindtalk_mode_change`, `mindtalk_message_send`, `mindtalk_tarot_draw`, `mindtalk_sprout_insufficient`, `mindtalk_login_click`, `mindtalk_new_round`) |
| 2026-03-11 | 나다움 분석/마음톡 페이지 타이틀 추가 (`/nadaum`, `/nadaum/:category`, `/maumtalk`) |
| 2026-03-11 | 데일리 마음 상담 프로세스 변경 대응: `consult_entry_click`(홈 진입), `consult_type_select_view`(유형 선택 페이지) 추가, `/consult-type-select` 타이틀 추가 |
| 2026-03-11 | `consult_start_click` 설명 변경: "홈에서 상담 시작" → "유형 선택 페이지에서 사주/타로 선택" |
