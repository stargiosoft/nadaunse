...
> **슬라이드**: 10장 (표지/Q&A 포함)
> **플로우**: 문제 → 증거 → 해결책 → 근거 → 기술 → 차별화+Lock-in → R&D+비즈니스 → 비전
> **데모**: 라이브 시연 (PPT 미포함)
...
### 슬라이드 7. 왜 나다운세인가 — 차별화 & 데이터 Lock-in

```
■ 경쟁 서비스 5종 비교

  │  서비스     │  분석 깊이  │  행동 예측  │  가이드  │  누적  │  Lock-in  │
  │─────────────│────────────│───────────│────────│──────│──────────│
  │  MBTI 테스트 │  유형 분류   │  ❌        │  ❌     │  ❌   │  없음     │
  │  아이작      │  척도 200+   │  ❌        │  ❌     │  ❌   │  없음     │
  │  트로스트    │  AI 상담     │  ❌        │  △     │  ❌   │  약함     │
  ...
  → 기존 서비스: "진단하고 끝" 또는 "점치고 끝"
  → 나다운세:    "진단 → 예측 → 가이드 → 추적" 풀 사이클

■ 데이터가 쌓일수록 떠날 수 없는 구조 (Lock-in)

  1회차: 온톨로지 12노드, 유형 1개
         → 전환비용 낮음 (떠나도 아깝지 않음)

  3회차: 온톨로지 28노드, 유형 3개, 적중률 데이터 축적
         → 전환비용 중간 ("3달치 데이터가 아까운데...")

  6회차: 온톨로지 45노드, 유형 4개, 적중률 6개월 추적
         → 전환비용 높음 ("이건 나만의 인생 지도인데")

  12회차: 1년치 심리 데이터, 계절별 패턴, 궁합 관계 맵
         → 사실상 이탈 불가 (다른 서비스에서 재현 불가)

■ 핵심: 피드백 루프가 만드는 자기강화

  예측 생성 → 행동 가이드 → 사용자 실제 행동
       ↑                          ↓
  AI 모델 개인화 ← 피드백 ("예측이 맞았나요?")

  쓸수록 → 데이터 축적 → 예측 정밀도 향상 → 더 유용 → 더 자주 사용
  = 네트워크 효과 없이도 개인 단위에서 Lock-in 발생
```
...
■ AI R&D 3대 과제
  ...
  과제 1: 행동 예측 AI 모델 고도화
    현재 LLM 프롬프트 기반 → 자체 Fine-tuned 모델
    77,898명 × 5M+ 메시지 학습 데이터 보유
    한국어 심리 도메인 특화 + 정확도 정량 측정

  과제 2: 심리 온톨로지 자동 구축 엔진
    HEXACO + 6개 척도 교차 분석 자동화
    GNN 기반 노드 관계 추론 + 시계열 비교

  과제 3: 멀티에이전트 시뮬레이션 고도화
    심리학 이론 기반 추론 규칙 + what-if 시나리오 분기
...
| "AI 예측 정확도?" | "현재 LLM 기반, 자체 모델 개발 + 적중률 정량 검증 구축 예정" |
| "심리학 전문가 자문?" | "한국어 타당화 완료 척도만 채택. 향후 자문위원 연계 계획" |
| "개인정보/윤리?" | "익명화, RLS 적용, 동의 기반 수집. 민감 정보 서버사이드 처리" |

### 라이브 데모 시연 순서 (슬라이드 6 이후)

```
1. nadaunse.com/future-prediction 접속
2. 카테고리 선택 (연애)
3. 태도 테스트 5문항 진행
4. 태그 선택 화면 보여주기
5. (미리 저장된 결과로) 온톨로지 그래프 시각화
6. 간극 분석 게이지
7. AI 에이전트 토론 결과
```
