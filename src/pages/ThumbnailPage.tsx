...
function buildCutPrompt(cutId: string, color: string, stoneName: string, gender?: 'female' | 'male', pose?: string, subType?: string, outfitOverride?: string, hasStoneRef?: boolean, bgConcept?: BackgroundConcept, bgItems?: string): string {
  ...
  switch (cutId) {
    case 'product': {
      if (subType === 'holder')
        return `${prefix}팔찌 홀더 제품 사진. 【홀더 착용 구조 강제】화이트 또는 크림 원통형 주얼리 디스플레이 롤(bracelet roll holder)에 팔찌가 끼워진 상태 — 원통이 팔찌의 원형 안쪽을 관통하여 팔찌가 원통 몸통 중앙을 고리처럼 감싸고 있는 구조. 팔찌의 비즈 라인이 원통 둘레를 따라 원형으로 자연스럽게 얹혀 있음. 팔찌가 원통 옆에 걸쳐지거나 늘어지거나 바닥에 닿는 것 절대 금지 — 반드시 원통 몸통을 통과해 끼워진 형태. ${scene.surface} 위에 롤을 비스듬히 놓고 위에서 살짝 내려다보는 앵글. 롤 옆에 ${scene.props}. 탁하거나 흐릿한 느낌 없이 선명하고 밝게. ${scene.light}. 한국 고급 주얼리 브랜드 상업 사진.`;
      if (subType === 'props')
