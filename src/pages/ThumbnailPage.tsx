...
function getHeavyScene(): { surface: string; props: string; light: string } {
  return {
    surface: '순백의 하이글로시 또는 매트 화이트 마블 슬랩 — 표면이 깨끗하고 밝으며 탁하거나 베이지·크림 기운 없이 순수한 화이트. 퓨어와 동일한 슬랩 구도',
    props: '프레임 한쪽 구석에 화이트 안개꽃(gypsophila) 또는 실버 팜파스 그라스 드라이플라워 한 줄기만. 목화솜(cotton flower)·코튼플라워·풍성한 흰 솜털 꽃봉오리 절대 금지',
    light: '밝고 선명한 자연광, 쿨 화이트 색온도. 표면이 깨끗하게 빛나도록 충분한 밝기 확보. 탁하거나 노르스름한 색조 없이 화이트·쿨그레이·블랙의 선명한 모노크롬 톤',
  };
}
