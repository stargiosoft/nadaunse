...
function getHeavyScene(): { surface: string; props: string; light: string } {
  return {
    surface: '거의 화이트에 가까운 밝은 연회색 매트 슬랩 — 수평 스트라이에이션과 미세 다크 스펙클이 분포된 깨끗한 표면. 슬랩 두께와 날카로운 모서리 비스듬히 보임. 배경은 밝고 클린한 연회색 플랫 서피스. 베이지·웜톤·더티그레이 절대 금지. 광택 전혀 없이 완전 매트',
    props: '프레임 한쪽 구석에 화이트 안개꽃(gypsophila) 또는 실버 팜파스 그라스 드라이플라워 한 줄기만. 목화솜(cotton flower)·코튼플라워·풍성한 흰 솜털 꽃봉오리 절대 금지',
    light: '대형 소프트박스 스튜디오 스트로브 — 45도 사이드에서 조사, 팔찌와 슬랩에 선명하고 방향감 있는 그림자 형성. 완벽한 화이트 밸런스 5500K, 색 캐스트(color cast) 전혀 없음. 선명하고 크리스프하게, 팔찌 원석 색상 채도 살아있게. 미디엄 포맷 카메라, 핀샤프. 스마트폰·아마추어·탁함·뿌연 느낌 절대 금지. 하이엔드 주얼리 스튜디오 화보',
  };
}
