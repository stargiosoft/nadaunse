...
function getHeavyScene(): { surface: string; props: string; light: string } {
  return {
    surface: '매트한 밝은 웜 그레이 콘크리트 정사각형 블록 오브제 — 팔찌를 블록 표면 위에 올려놓은 아키텍처럴 미니멀 구도. 배경은 따뜻한 베이지/웜 그레이 플랫 서피스',
    props: '실물 소품 없음. 자연광이 만들어내는 식물 잎 보케 그림자가 배경과 블록 옆에 소프트하게 드리워짐',
    light: '따뜻한 자연 사이드 라이팅 — 부드러운 햇빛이 한 방향에서 들어와 식물 잎 그림자 보케를 배경에 만들어냄. 전체적으로 밝고 클린하되 따뜻한 온도감. 모던 에디토리얼 주얼리 화보 스타일',
  };
}

function getNaturalScene(): { surface: string; props: string; light: string } {
  return {
    surface: '밝은 오크 또는 애쉬 원목 슬라이스 — 나이테가 섬세하게 보이는 모던 에디토리얼 스타일의 원목 오브제 위에 팔찌 배치. 배경은 따뜻한 아이보리/크림 플랫 서피스',
    props: '실물 소품 없음. 또는 작은 유칼립투스 잎 한두 장이 원목 옆에 자연스럽게 놓임',
    light: '부드럽고 확산된 야외 자연광 — 따뜻하고 클린한 톤. 플랜트 그림자 보케가 배경에 살짝 드리워지는 모던 라이프스타일 무드',
  };
}
