...
function getHeavyScene(): { surface: string; props: string; light: string } {
  return {
    surface: '화이트 콘크리트 블록 슬랩 — 표면 전체에 작은 기공(pore)과 거친 질감이 실제 콘크리트처럼 리얼하게 보임. 불규칙한 거친 패치, 미세한 구멍들, 흰색~라이트그레이 색조 변화가 자연스럽게 분포. 슬랩 두께와 모서리가 비스듬히 보이는 구도. 배경은 라이트 그레이 플랫 서피스. 광택·반사 완전 없이 매트',
