...
function getHeavyScene(): { surface: string; props: string; light: string } {
  return {
    surface: '라이트 그레이 시멘트 보드 슬랩 — 표면이 매끄럽고 평탄하되, 수평 방향의 은은한 줄무늬(striation)와 미세한 블랙·다크그레이 스펙클이 전체에 자연스럽게 분포. 거칠지 않고 부드러운 질감이나 실제 시멘트 재질감이 사실적으로 표현됨. 슬랩 두께와 모서리가 비스듬히 보이는 구도. 배경은 클린한 라이트 그레이 플랫 서피스. 광택·반사 없이 완전 매트',
