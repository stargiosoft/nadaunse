...
type BackgroundConcept = 'pure' | 'heavy' | 'natural' | 'dark' | 'sand' | 'ocean';
...
function getHeavyScene(): { surface: string; props: string; light: string } {
  return {
    surface: '순백(pure white) 매트 슬랩 — 흰 표면 위에 수평 방향 스트라이에이션 여러 줄이 선명하게 평행 분포하고, 크고 작은 블랙 스펙클 점들이 흰 표면 위에 뚜렷하게 찍혀 있음. 슬랩 두께와 모서리 비스듬히 전면에 보임. 배경은 클린한 라이트그레이 플랫 서피스. 베이지·웜톤·회색 기운 절대 금지. 광택·반사 전혀 없이 완전 매트',
    props: '프레임 한쪽 구석에 화이트 안개꽃(gypsophila) 또는 실버 팜파스 그라스 드라이플라워 한 줄기만',
    light: '상업 제품 사진 스튜디오 세팅 — 창문광·자연광 없이 스트로브 단독 조명. 45도 사이드 대형 소프트박스로 팔찌와 슬랩에 방향감 있고 선명한 그림자. 완벽한 화이트 밸런스, 색 캐스트(color cast) 전혀 없음. 탁함·노이즈·뿌연 느낌 없이 크리스프하고 선명. 팔찌 원석 색상 채도 풍부하게. 미디엄 포맷 카메라. 하이엔드 상업 주얼리 카탈로그 사진. 목화솜(cotton flower)·코튼플라워·풍성한 흰 솜털 꽃봉오리 절대 금지',
  };
}
...
function getDarkScene(): { surface: string; props: string; light: string } {
  return {
    ...
    props: '매끄럽게 다듬어진 블랙 리버 스톤(강돌) 2–3개',
    light: '스튜디오 스트로브 사이드 조명 — 창문광·자연광 없이 스트로브 단독. 【제품 노출 최우선】팔찌에는 밝고 집중된 스폿 하이라이트를 충분히 줘 원석 색상·비즈 디테일이 선명하게 보일 것 — 팔찌 자체가 어둡게 묻히는 것 절대 금지. 배경과 슬레이트는 미디엄 다크로 유지해 팔찌와 대비를 만들 것. 배경이 칠흑(pure black)이 되지 않도록. 완벽한 화이트 밸런스, 탁함·뿌연 느낌 없이 크리스프. 하이엔드 주얼리 스튜디오 화보. 목화솜(cotton flower)·코튼플라워·안개꽃·팜파스·드라이플라워·꽃 종류 일체 절대 금지',
  };
}

function getOceanScene(): { surface: string; props: string; light: string } {
  return {
    surface: '밝은 블루-아쿠아 빛이 감도는 트래버틴 스톤 슬랩 — 크림/화이트 베이스에 연한 바다빛 청록(aqua) 톤이 자연스럽게 스며든 대리석 슬랩. 배경은 소프트 아쿠아-화이트 플랫 서피스. 광택 없이 완전 매트. 실제 바다·파도·모래사장·물 절대 금지',
    props: '프레임 한쪽 구석에 흰색 조개껍데기(white shell) 1~2개 또는 작은 불가사리(starfish) 1개만',
    light: '밝고 부드러운 스튜디오 소프트박스 조명 — 45도 사이드 라이트. 슬랩의 청록 빛이 은은하게 살아나도록. 전체적으로 밝고 청량한 아쿠아-화이트 팔레트. 하이엔드 주얼리 상업 카탈로그 사진',
  };
}
...
function buildCutPrompt(cutId: string, color: string, stoneName: string, gender?: 'female' | 'male', pose?: string, subType?: string, outfitOverride?: string, hasStoneRef?: boolean, bgConcept?: BackgroundConcept, bgItems?: string): string {
  ...
  const fidelityNote = hasStoneRef
    ? `【제품 리스테이징 — 팔찌 디자인 창작 금지】이 요청은 레퍼런스 이미지의 팔찌를 새 배경·구도에 재배치하는 것임. 팔찌는 레퍼런스 이미지의 것 단 1개만 배치할 것 — 추가 팔찌·다른 디자인의 팔찌 절대 금지. 팔찌 자체를 새로 디자인하거나 재해석하지 말 것. 비즈 색상·배열·크기·형태·소재·골드 스페이서를 레퍼런스와 100% 동일하게 복사할 것. ${hardwareLock}팔찌의 원석/펜던트 디자인은 별도 원석 레퍼런스 기준으로 교체. 변경 허용: 배경 표면·소품·조명·카메라 앵글만. 【촬영 기준】한국 고급 주얼리 브랜드 화보. 소프트박스 조명, 미디엄 포맷 카메라. 핀포커스, 완벽한 노출. `
    : `【제품 리스테이징 — 팔찌 디자인 창작 금지】이 요청은 레퍼런스 이미지의 팔찌를 새 배경·구도에 재배치하는 것임. 팔찌는 레퍼런스 이미지의 것 단 1개만 배치할 것 — 추가 팔찌·다른 디자인의 팔찌 절대 금지. 팔찌 자체를 새로 디자인하거나 재해석하지 말 것. 비즈 색상·배열·크기·형태·소재를 레퍼런스와 100% 동일하게 복사할 것. ${hardwareLock}변경 허용: 배경 표면·소품·조명·카메라 앵글만. 팔찌 원본은 픽셀 단위로 동일하게 재현할 것. 【촬영 기준】한국 고급 주얼리 브랜드 화보. 소프트박스 조명, 미디엄 포맷 카메라. 핀포커스, 완벽한 노출. `;
  const prefix = fidelityNote + (productLabel ? `${productLabel} ` : '');
  const rawScene = bgConcept === 'heavy'
    ? getHeavyScene()
    : bgConcept === 'natural'
      ? getNaturalScene()
      : bgConcept === 'dark'
  ...
  switch (cutId) {
    case 'product': {
      if (subType === 'holder')
        return `${prefix}팔찌 홀더 제품 사진. 【★비즈·배열 절대 고정 — 구도보다 최우선★】레퍼런스 팔찌의 비즈 색상·순서·배열·크기·소재·클라스프·금속 부속을 원통에 감은 상태에서도 100% 동일하게 재현. 원통에 감겼을 때 보이지 않는 뒷면 비즈를 제외한 모든 가시 비즈는 레퍼런스와 완전히 일치해야 함. "홀더에 어울리는 팔찌"를 새로 디자인하지 말 것 — 레퍼런스 팔찌를 그대로 원통에 올린 것뿐임. 【완성 이미지 묘사 — 이 비주얼을 정확히 재현할 것】화이트 원통형 주얼리 디스플레이 롤이 ${scene.surface} 위에 가로로 놓여 있음. 팔찌는 원통 몸통 중앙에 딱 맞게 위치해, 원통 전면에서 바라봤을 때 팔찌 비즈 라인이 원통 표면을 가로지르는 수평 띠(horizontal band)처럼 보임 — 마치 원통 허리에 벨트를 두른 것처럼 팔찌가 원통 표면에 밀착해 수평으로 한 바퀴 돌아 있음. 팔찌가 원통 중앙부에서 위아래로 흘러내리거나 비스듬히 처지거나 원통 밖으로 벗어나는 것 절대 금지. 살짝 위에서 내려다보는 앵글. 롤 옆에 ${scene.props}. 탁하거나 흐릿한 느낌 없이 선명하고 밝게. ${scene.light}. 한국 고급 주얼리 브랜드 상업 사진.`;
      if (subType === 'props')
      ...
      // flatlay (default)
      return `${prefix}팔찌 플랫레이 제품 사진. ${scene.surface} 위에 팔찌를 완전한 정원형으로 올려놓고 정수리 방향에서 수직으로 내려다보는 오버헤드 앵글. ${bgItems?.trim() ? `프레임 한쪽 구석에 ${scene.props}를 자연스럽게 배치.` : '소품 없이 팔찌만, 미니멀하고 깔끔한 구도.'} ${scene.light}. 럭셔리 주얼리 상업 사진.`;
    }
    case 'wearing': {
  ...
  const handleBatchProductGenerate = useCallback(async () => {
    ...
    const tasks = PRODUCT_CUTS.flatMap((cut, ci) =>
      Array.from({ length: PER_CUT }, (_, si) => ({
        ...
        itemPrompt: buildCutPrompt('product', productColor, stoneName, undefined, undefined, cut.id, undefined, !!stoneRefBase64, bgConcept, bgItems),
    ...
    setGenerating(false);
  }, [productColor, stoneName, stoneRefBase64, bgConcept, bgItems, referenceBase64s, referenceMode]);
                  ...
                  <select
                    ...
                    onChange={e => {
                      ...
                      if (activeCutPreset) {
                        ...
                        setPrompt(buildCutPrompt(activeCutPreset, prod.color, prod.stone, g, po, sub, ov, !!stoneRefBase64, bgConcept, bgItems));
                      }
                    }}
                    className="outline-none"
                    style={{
                  ...
                  {PRODUCT_CUTS.map(a => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          setProductCut(a.id);
                          setPrompt(buildCutPrompt('product', productColor, stoneName, undefined, undefined, a.id, undefined, !!stoneRefBase64, bgConcept, bgItems));
                        }}
              ...
              {shootMode === 'product' && (activeCutPreset === 'product' || activeCutPreset === 'detail') && (
                <div style={{ marginTop: '8px' }}>
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            setPrompt(buildCutPrompt(activeCutPreset, productColor, stoneName, undefined, undefined, sub, undefined, !!stoneRefBase64, c.id, bgItems));
                          }}
                  ...
                  <input
                    ...
                    value={bgItems}
                    onChange={e => {
                      setBgItems(e.target.value);
                      const sub = activeCutPreset === 'product' ? productCut : undefined;
                      setPrompt(buildCutPrompt(activeCutPreset!, productColor, stoneName, undefined, undefined, sub, undefined, !!stoneRefBase64, bgConcept, e.target.value));
                    }}
                    placeholder="배경 소품 입력 (예: 조개껍데기, 드라이플라워, 레몬 슬라이스)"
                    className="outline-none"
                    style={{
                      width: '100%', height: '30px', borderRadius: '10px',
                      border: `1px solid ${bgItems ? C.primary : C.borderDefault}`,
                      padding: '0 10px', marginTop: '4px',
                      fontFamily: font, fontSize: '11px', fontWeight: 400,
                      color: C.textPrimary,
                      backgroundColor: bgItems ? '#f0fafa' : C.surface,
                      boxSizing: 'border-box',
                    }}
                  />
                </div>
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            setPrompt(buildCutPrompt('wearing', productColor, stoneName, g, wearingPose, undefined, ov, !!stoneRefBase64, bgConcept, bgItems));
                          }}
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            setPrompt(buildCutPrompt('wearing', productColor, stoneName, wearingGender, p.id, undefined, ov2, !!stoneRefBase64, bgConcept, bgItems));
                          }}
                          ...
                          {(() => {
                            ...
                            return (
                              <button
                                onClick={() => {
                                  setWearingOutfitId('');
                                  setPrompt(buildCutPrompt('wearing', productColor, stoneName, wearingGender, wearingPose, undefined, '', !!stoneRefBase64, bgConcept, bgItems));
                                }}
                                style={{
                            ...
                            return (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  setPrompt(buildCutPrompt('wearing', productColor, stoneName, wearingGender, wearingPose, undefined, o.outfit, !!stoneRefBase64, bgConcept, bgItems));
                                }}
                                style={{
                                  height: '24px', borderRadius: '6px',
                  ...
                  {WHITE_TYPES.map(w => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          setWhiteType(w.id);
                          setPrompt(buildCutPrompt('white', productColor, stoneName, undefined, undefined, w.id, undefined, !!stoneRefBase64, bgConcept, bgItems));
                        }}
